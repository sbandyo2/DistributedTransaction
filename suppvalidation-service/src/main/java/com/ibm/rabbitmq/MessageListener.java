...
public class MessageListener {
    ...
    public void receiveMessageForApp1(final SupplierInfo info) {
    	log.info("Received message: {} from app1 queue.", info);
    	String uri = "http://localhost:9103/suppliervalidation/validate";
    	try {
    		log.info("Making REST call to the API");
    		if(ApplicationConstant.VALIDATE_SUPP_EVENT.equalsIgnoreCase(info.eventId)){
    			if("US".equalsIgnoreCase(info.getCountry())){
    				
    				if("NY".equalsIgnoreCase(info.getRegion())){
    					
    					info.setEventId(ApplicationConstant.VALID_EVENT);
    				    
    				} else {
    					info.setEventId(ApplicationConstant.INVALID_EVENT);
    				}
    			}
